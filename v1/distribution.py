...
class AngularCentralGaussianMultiView():
    def __init__(self, scale_tril, df):
        self.scale_tril = scale_tril
        self.nV = scale_tril.shape[-3]
        self.distri = [AngularCentralGaussian(scale_tril[...,i,:,:]) for i in range(self.nV)]
    
    def __call__(self,sample_shape=torch.Size()):
        return torch.stack([self.distri[i](sample_shape) for i in range(self.nV)],dim=-2)

class MultivariateStudentTMultiView():
    def __init__(self, loc, scale_tril, df):
        self.loc = loc
        self.scale_tril = scale_tril
        self.df = df
        self.nV = scale_tril.shape[-3]
        self.distri = [ MultivariateStudentT(loc=self.loc[...,i,:], scale_tril = scale_tril[...,i,:,:],df=self.df) for i in range(self.nV)]
